...
export class SceneHello {
  ...
  async onEnter(@Ctx() ctx: Scenes.WizardContext & ITelegrafI18nContext<I18nTranslations>) {
    const translated_message = ctx.translate(i18nKeys.i18n.command.hello.scene.first_message)
    await ctx.reply(translated_message);
  }

  @WizardStep(1)
  async step1(@Ctx() ctx: Scenes.WizardContext & ITelegrafI18nContext<I18nTranslations>) {
    const translated_message = ctx.translate(i18nKeys.i18n.command.hello.scene.second_message)
    await ctx.reply(translated_message);
